...
app.use('/search', mealdbRoutes);

describe('MealDB Routes', () => {
  test('GET /search/search should return meal data', async () => {
    const res = await request(app).get('/mealdb/search?s=Arrabiata');
    expect(res.statusCode).toBe(200);
    expect(res.body).toHaveProperty('meals');
  });

  test('GET /search/search-by-letter should return meals by letter', async () => {
    const res = await request(app).get('/mealdb/search-by-letter?f=a');
    expect(res.statusCode).toBe(200);
    expect(res.body).toHaveProperty('meals');
  });

  test('GET /search/lookup should return meal details by id', async () => {
